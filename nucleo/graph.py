...
class Graph:
    ...
    def obter_vizinhos(self, no):
        ...
        return [vizinho for vizinho, custo in self.adjacencias[no]]
...
def gerar_grafo_labirinto_com_biomas(tamanho_min=30, largura=20, altura=15, 
                                    seed=None, scale_biomas=0.08, num_recompensas_min=5):
    """
    ...
    biome_map, biome_stats = gerar_biomas_naturais(
        largura, altura, seed=seed, scale=scale_biomas, octaves=3
    )
    ...
    for x, y in valid_positions:
        ...
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if (nx, ny) in valid_positions:
                ...
                if vizinho and vizinho not in grafo.obter_vizinhos(no_atual):
...
def gerar_grafo_teste(tamanho_min=30, largura=20, altura=15):
    """
    Função de compatibilidade - usa novo sistema com configurações padrão
    """
    resultado = gerar_grafo_labirinto_com_biomas(tamanho_min, largura, altura)
    if resultado is None:
        return None
    return resultado[0]  # Retorna apenas o grafo para compatibilidade

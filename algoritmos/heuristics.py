...
def heuristica_terreno(no_atual, objetivo, grafo=None):
    ...
    # Fatores de penalidade baseados no tipo de terreno
    fatores_terreno = {
        TipoTerreno.SOLIDO: 1.0,
        TipoTerreno.ARENOSO: 2.0,
        TipoTerreno.ROCHOSO: 4.0,
        TipoTerreno.PANTANO: 8.0
    }
    
    # Se temos acesso ao grafo, calculamos penalidade baseada no terreno atual
    if grafo:
        # Considera o terreno atual e uma estimativa do caminho
        fator_atual = fatores_terreno.get(no_atual.tipo_terreno, 1.0)
        
        # Estimativa simples: assume terreno médio para o resto do caminho
        fator_medio = 2.5  # Valor médio entre os terrenos
        
        return distancia_base * (fator_atual * 0.3 + fator_medio * 0.7)
    else:
        # Sem grafo, usa fator médio conservativo
        return distancia_base * 2.0

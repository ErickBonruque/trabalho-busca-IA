...
def inicializar_ambiente():
    ...
    resultado = gerar_grafo_labirinto_com_biomas(
        tamanho_min=50, largura=25, altura=15, seed=None
    )
        ...
        while True:
            ...
            if opcao == "sair":
                ...
                if ultimos_resultados:
                    ...
                    if algoritmo and algoritmo in ultimos_resultados:
                        ...
                        if resultado.sucesso:
                            ...
                            try:
                                velocidade = float(input("Velocidade (segundos por passo, padrão 1.0): ") or "1.0")
                            except:
                                velocidade = 1.0
                ...
                if ultimo_comparativo:
                    nome_arquivo = input("Nome do arquivo (padrão: relatorio_busca.txt): ").strip()
                    if not nome_arquivo:
                        nome_arquivo = "relatorio_busca.txt"
                    
                    # Garante que será salvo na pasta resultados
                    relatorio_path = f"resultados/{nome_arquivo}"
                    mapa_path = f"resultados/mapa_{nome_arquivo.replace('.txt', '')}.txt"
